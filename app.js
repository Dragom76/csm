...
// ════════════════════════════════════════════════════════════
//  DB 연결 (Supabase Session Pooler - IPv4 호환)
// ════════════════════════════════════════════════════════════
const pool = new Pool(
  process.env.DATABASE_URL
    ? { connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } }
    : {
        host:     `aws-1-${process.env.SUPABASE_REGION || 'ap-southeast-2'}.pooler.supabase.com`,
        port:     5432,
        user:     `postgres.${(process.env.SUPABASE_URL || '').replace('https://', '').replace('.supabase.co', '')}`,
...
// ── PostgreSQL 컬럼명 소문자 → 프론트 호환 대문자 변환 ────────────
...
app.use(session({
  ...
  secret: process.env.SESSION_SECRET || 'dragom-erp-fallback-2026',
  resave: false,
...
async function uploadFile(file) {
  ...
  await r2.send(new PutObjectCommand({
    Bucket:      process.env.R2_BUCKET_NAME || 'csm',
    Key:         fname,
    Body:        file.buffer,
...
function requireAuth(roles) {
  return (req, res, next) => {
    ...
    if (!u) return req.path.startsWith('/api')
      ...
      : res.redirect('/login');
    if (u.status !== 'ACTIVE') return req.path.startsWith('/api')
      ? res.status(403).json({ error: '승인 대기 중' })
...
// ════════════════════════════════════════════════════════════
//  DB 초기화 (ADMIN 계정 + 기본 프로젝트 시드)
// ════════════════════════════════════════════════════════════
async function initDb() {
  try {
    const adminPw = 'Dragom76!!';
...
function httpsPost(url, body, headers = {}) {
  return new Promise((resolve, reject) => {
    const u    = new URL(url);
    const data = Buffer.from(typeof body === 'string' ? body : new URLSearchParams(body).toString());
...
<p style="font-size:.8rem;color:#9ca3af">이 창은 자동으로 닫힙니다</p></div>
<script>try{if(window.opener)window.opener.postMessage(${JSON.stringify(data)},location.origin)}catch(e){}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    const dup = await pool.query('SELECT 1 FROM tboard_users WHERE user_id=$1', [userId]);
    if (dup.rows.length) return res.status(409).json({ error: '이미 사용 중인 아이디입니다.' });
    await pool.query(
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    const r = await pool.query(
      'SELECT user_id,user_pw,user_name,role,status FROM tboard_users WHERE user_id=$1', [userId]
    );
...
app.get('/api/auth/google/callback', async (req, res) => {
  ...
  try {
    ...
    if (mode === 'verify') {
      return res.send(socialResultHtml({ success: true, type: 'socialVerified', provider: 'google',
        key: userInfo.id, name: userInfo.name, email: userInfo.email }));
    }
    const r = await pool.query(
  ...
  } catch(e) { console.error('Google OAuth:', e.message); res.send(socialResultHtml({ success: false, error: '인증 처리 중 오류' })); }
});
...
app.get('/api/auth/kakao/callback', async (req, res) => {
  ...
  try {
    const params = new URLSearchParams({ code, grant_type: 'authorization_code',
      client_id: OAUTH.kakao.clientId, redirect_uri: OAUTH.kakao.redirect });
    const token    = await httpsPost('https://kauth.kakao.com/oauth/token', params.toString());
    const userInfo = await httpsGet('https://kapi.kakao.com/v2/user/me',
      { Authorization: `Bearer ${token.access_token}` });
    const kakaoId   = String(userInfo.id);
    const kakaoName = userInfo.kakao_account?.profile?.nickname || '카카오사용자';
    if (mode === 'verify') {
      return res.send(socialResultHtml({ success: true, type: 'socialVerified', provider: 'kakao',
        key: kakaoId, name: kakaoName, email: userInfo.kakao_account?.email || '' }));
    }
    const r = await pool.query(
...
app.get('/api/auth/naver/callback', async (req, res) => {
  ...
  try {
    const params = new URLSearchParams({ grant_type: 'authorization_code',
      client_id: OAUTH.naver.clientId, client_secret: OAUTH.naver.clientSecret,
      code, state: req.session.naverState || '' });
    const token    = await httpsPost('https://nid.naver.com/oauth2.0/token', params.toString());
    const userInfo = await httpsGet('https://openapi.naver.com/v1/nid/me',
      { Authorization: `Bearer ${token.access_token}` });
    const profile   = userInfo.response;
    const naverId   = profile.id;
    const naverName = profile.name || '네이버사용자';
    if (mode === 'verify') {
      return res.send(socialResultHtml({ success: true, type: 'socialVerified', provider: 'naver',
        key: naverId, name: naverName, email: profile.email || '' }));
    }
    const r = await pool.query(
...
app.get('/api/users/:id/pw', requireAuth(['ADMIN']), async (req, res) => {
  try {
    ...
    if (!r.rows.length) return res.status(404).json({ error: '없음' });
    res.json({ pw: decryptPw(r.rows[0].user_pw_enc || '') || '(복호화 불가)' });
  ...
  try {
    const r = await pool.query(
      `SELECT p.project_id, p.project_name,
       ...
       WHERE p.status='ACTIVE' ORDER BY p.project_name`,
      [req.params.id]
    );
    res.json(toUpper(r.rows));
  } catch(e) { console.error(e); res.status(500).json({ error: '조회 실패' }); }
    ...
    if (isAdmin) {
      r = await pool.query(
        `SELECT p.project_id, p.project_name, p.project_code, p.description, p.status,
                p.created_by, TO_CHAR(p.created_at,'YYYY-MM-DD') created_at,
                (SELECT COUNT(*) FROM board b WHERE b.project_id=p.project_id) post_count,
                (SELECT COUNT(*) FROM project_members pm WHERE pm.project_id=p.project_id) member_count
      ...
      r = await pool.query(
        `SELECT p.project_id, p.project_name, p.project_code, p.description, p.status,
                p.created_by, TO_CHAR(p.created_at,'YYYY-MM-DD') created_at,
                (SELECT COUNT(*) FROM board b WHERE b.project_id=p.project_id) post_count,
                (SELECT COUNT(*) FROM project_members pm2 WHERE pm2.project_id=p.project_id) member_count
         FROM projects p
  ...
  try {
    const r = await pool.query(
      `SELECT u.user_id, u.user_name, u.role, u.status,
              TO_CHAR(pm.joined_at,'YYYY-MM-DD') joined_at
    ...
    const r = pid
      ? await pool.query(
          `SELECT id,title,content,image_path,author_id,author_name,
                  TO_CHAR(created_at,'YYYY-MM-DD HH24:MI') created_at
           FROM board WHERE project_id=$1 ORDER BY id DESC`, [pid])
      : await pool.query(
          `SELECT id,title,content,image_path,author_id,author_name,
                  TO_CHAR(created_at,'YYYY-MM-DD HH24:MI') created_at
...
app.get('/api/board/:id', requireAuth(), async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT id,title,content,image_path,author_id,author_name,
              TO_CHAR(created_at,'YYYY-MM-DD HH24:MI') created_at
       FROM board WHERE id=$1`, [req.params.id]
    );
    if (!r.rows.length) return res.status(404).json({ error: '없음' });
    res.json(toUpper(r.rows)[0]);
  } catch(e) { res.status(500).json({ error: '조회 실패' }); }
...
app.post('/api/board', requireAuth(), upload.single('image'), async (req, res) => {
  ...
  let imagePath = null;
  if (req.file) {
    try { imagePath = await uploadFile(req.file); }
    catch(e) { return res.status(500).json({ error: '이미지 업로드 실패' }); }
  }
  try {
    await pool.query(
      `INSERT INTO board(title,content,image_path,project_id,author_id,author_name)
       VALUES($1,$2,$3,$4,$5,$6)`,
      [title, content, imagePath, projectId || 1, req.session.user.id, req.session.user.name]
    );
    res.json({ success: true });
...
app.put('/api/board/:id', requireAuth(), upload.single('image'), async (req, res) => {
  ...
  try {
    if (req.file) {
      ...
      await pool.query('UPDATE board SET title=$1,content=$2 WHERE id=$3', [title, content, req.params.id]);
    }
    res.json({ success: true });
  } catch(e) { console.error(e); res.status(500).json({ error: '수정 실패' }); }
});
  ...
  try {
    await pool.query(
      `INSERT INTO comments(post_id,author_id,author_name,content,image_path)
       VALUES($1,$2,$3,$4,$5)`,
      [req.params.id, u.id, u.name, req.body.content || null, imagePath]
    );
    res.json({ success: true });
  } catch(e) { console.error(e); res.status(500).json({ error: '등록 실패' }); }
});
